...
TEMP_INPUT=$(mktemp)
echo "2" > "$TEMP_INPUT"
echo "./test_files/input3.txt" >> "$TEMP_INPUT"

# Check if input and answer files exist
#ile_exists ./test_files/input3.txt
#file_exists ./test_files/answer3.txt
...
sed -E 's/  +/ /g; s/^ +| +$//g' ./test_files/answer3.txt > cleaned_answer.txt
